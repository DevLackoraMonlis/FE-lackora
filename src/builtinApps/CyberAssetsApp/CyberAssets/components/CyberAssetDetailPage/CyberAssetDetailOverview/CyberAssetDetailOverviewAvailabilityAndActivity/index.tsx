...
import { Flex, ScrollArea, SegmentedControl, Text, Timeline, Tooltip } from "@mantine/core";
	...
	const selectedPanel: Record<SelectPanelType, ReactElement> = {
		"Availability Timeline": (
			<Timeline bulletSize={20} lineWidth={2}>
				{props.availabilityAndActivity.availability.map((item) => (
					<Timeline.Item
			...
			<Timeline bulletSize={20} lineWidth={2}>
				{props.availabilityAndActivity.activity.map((item) => (
					<Timeline.Item
						...
						title={
							<Tooltip label={`${item.title} - ${item.time}`}>
								<Text maw={250} truncate={"end"} fz={"sm"} fw={"bold"}>{`${item.title} - ${item.time}`}</Text>
							</Tooltip>
						}
					>
						<Text fz={"xs"}>{item.description}</Text>
